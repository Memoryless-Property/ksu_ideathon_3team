...
def view_home():
    ...
    if st.button(btn_text, type="primary", use_container_width=True): go_to('write'); st.rerun()
...
def view_report():
    ...
    if sel_data:
        ...
        if "3단계" in level or "4단계" in level:
            if st.button("🚨 전문 기관 연결하기", type="primary", use_container_width=True): st.info("실제 앱에서는 109 전화 다이얼로 즉시 연결됩니다.")
    else: st.info(f"선택하신 {m}월 {d}일의 기록이 없습니다.\n홈 화면에서 먼저 작성해주세요.")
